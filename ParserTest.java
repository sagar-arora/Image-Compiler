...
public void testImage() throws LexicalException, SyntaxException {
	String input = "prog{image 	abc[x**2, y**4];}";
	show(input);
...
@Test
public void testStatemetntInput() throws LexicalException, SyntaxException {
	String input = "prog{input abc from @<<a,b,c,d>>;}";
	show(input);
	Scanner scanner = new Scanner(input).scan(); 
	show(scanner); 
	Parser parser = new Parser(scanner);
...
public void testUnaryExpression() throws LexicalException, SyntaxException {
	String input = "prog{input abc from @ -1;}";
	show(input);
	Scanner scanner = new Scanner(input).scan(); 
	show(scanner); 
	Parser parser = new Parser(scanner);
...
public void testPixelSelector() throws LexicalException, SyntaxException {
	String input = "prog{show atan(a>b);if(a!=b%c <= d){};show Z; show abc[x,y];}";
	show(input);
	Scanner scanner = new Scanner(input).scan(); 
	show(scanner); 
	Parser parser = new Parser(scanner);
