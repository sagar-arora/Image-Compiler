...
public class Scanner {
	...
	public static enum Kind {
		...
		RBRACE /* } */, LPIXEL /* << */, RPIXEL /* >> */, SEMI/* ; */, COMMA/* , */, DOT /* . */, KW_sleep, EOF;
	}
	...
	Scanner(String inputString) {
	 ...
	 private enum State {START, HAVE_EQUAL, DIGITS, HAVE_ZERO, IDENTIFIER, LPIXEL, RPIXEL, COLON, MUL, HAVE_NOTEQUAL, INTEGER, FLOAT, ZERO_INTERM, DIV, COMMENT};  //TODO:  this is incomplete
	 String[] keywords ={"Z","default_width","default_height","show","write" , "to" , "input" ,"from" ,"cart_x","cart_y","polar_a","polar_r","abs","sin" , "cos" , "atan" , "log" , "image" , "int" , "float" , "filename"
			 ,"boolean", "red" , "blue" , "green" , "alpha" , "while" , "if" , "width" , "height","int", "sleep"};
	 ArrayList<String> list=new ArrayList<>();
	 
	 public void putKeyWords(){
			...
			switch(state) {
				case START: {
					...
					switch (ch) {
						...
						break;
						case '(':{tokens.add(new Token(Kind.LPAREN, startPos, pos - startPos + 1));
						pos++;}
						break;
						case ')':{tokens.add(new Token(Kind.RPAREN, startPos, pos - startPos + 1));
									pos++;}
						break;	
						case '{':{tokens.add(new Token(Kind.LBRACE, startPos, pos - startPos + 1));
						pos++;}
						break;	
						case '}':{tokens.add(new Token(Kind.RBRACE, startPos, pos - startPos + 1));
						pos++;}
						break;	
						case '[':{tokens.add(new Token(Kind.LSQUARE, startPos, pos - startPos + 1));
						pos++;}
						break;	
						case ']':{tokens.add(new Token(Kind.RSQUARE, startPos, pos - startPos + 1));
						pos++;}
						break;
						case '<':{
							state=State.LPIXEL;
						...
						default: {
							if(ch=='.' && Character.isDigit(chars[pos+1])){
								state=State.FLOAT;
								pos++;
							}
							else if(ch=='.' && !Character.isDigit(chars[pos+1])){
				...
				case INTEGER:
				{
					if(Character.isDigit(ch)){
						...
						Token t=new Token(Kind.INTEGER_LITERAL,startPos,pos-startPos);
						try{
						...
						{	tokens.add(new Token(Kind.INTEGER_LITERAL,startPos,pos-startPos));
							...
							error(pos,line(pos),posInLine(pos),"Integer value greater than the range of Int");
						}
						}catch(NumberFormatException e){
							error(startPos,line(startPos),posInLine(startPos),"Integer value greater than the range of Int");
						}
					}
				}
				break;
				case FLOAT:{
					if(Character.isDigit(ch)) pos++;
						...
						if(Float.isFinite(t.floatVal()))
							{
							tokens.add(t);
							state=State.START;
							}
						else{
							error(startPos,line(startPos),posInLine(startPos),"Float Value out of Range");
						}
				...
				case IDENTIFIER:
				{
					if((Character.isLetterOrDigit(ch)|| ch=='_' || ch=='$')&& ch!=EOFChar ){
						pos++;
					}else{
						String value=String.copyValueOf(chars, startPos, pos-startPos);
						if(!list.contains(value)){
							if (value.equals("true")|| value.equals("false"))tokens.add(new Token(Kind.BOOLEAN_LITERAL,startPos, pos-startPos));
							else tokens.add(new Token(Kind.IDENTIFIER,startPos,pos-startPos));
						}else{
							if(value.equals("Z"))tokens.add(new Token(Kind.KW_Z,startPos, pos-startPos));
							if(value.equals("default_width")) tokens.add(new Token(Kind.KW_default_width,startPos, pos-startPos));
							if(value.equals("default_height")) tokens.add(new Token(Kind.KW_default_height,startPos, pos-startPos));
							if(value.equals("float")) tokens.add(new Token(Kind.KW_float,startPos, pos-startPos));
							if(value.equals("boolean")) tokens.add(new Token(Kind.KW_boolean,startPos, pos-startPos));
							if(value.equals("show")) tokens.add(new Token(Kind.KW_show,startPos, pos-startPos));
							if(value.equals("write")) tokens.add(new Token(Kind.KW_write,startPos, pos-startPos));
							if(value.equals("to")) tokens.add(new Token(Kind.KW_to,startPos, pos-startPos));
							if(value.equals("input")) tokens.add(new Token(Kind.KW_input,startPos, pos-startPos));
							if(value.equals("from")) tokens.add(new Token(Kind.KW_from,startPos, pos-startPos));
							if(value.equals("cart_x")) tokens.add(new Token(Kind.KW_cart_x,startPos, pos-startPos));
							if(value.equals("cart_y")) tokens.add(new Token(Kind.KW_cart_y,startPos, pos-startPos));
							if(value.equals("polar_a")) tokens.add(new Token(Kind.KW_polar_a,startPos, pos-startPos));
							if(value.equals("polar_r")) tokens.add(new Token(Kind.KW_polar_r,startPos, pos-startPos));
							if(value.equals("abs")) tokens.add(new Token(Kind.KW_abs,startPos, pos-startPos));
							if(value.equals("sin")) tokens.add(new Token(Kind.KW_sin,startPos, pos-startPos));
							if(value.equals("cos")) tokens.add(new Token(Kind.KW_cos,startPos, pos-startPos));
							if(value.equals("atan")) tokens.add(new Token(Kind.KW_atan,startPos, pos-startPos));
							if(value.equals("log")) tokens.add(new Token(Kind.KW_log,startPos, pos-startPos));
							if(value.equals("image")) tokens.add(new Token(Kind.KW_image,startPos, pos-startPos));
							if(value.equals("int")) tokens.add(new Token(Kind.KW_int,startPos, pos-startPos));
							if(value.equals("filename")) tokens.add(new Token(Kind.KW_filename,startPos, pos-startPos));
							if(value.equals("height")) tokens.add(new Token(Kind.KW_height,startPos, pos-startPos));
							if(value.equals("red")) tokens.add(new Token(Kind.KW_red,startPos, pos-startPos));
							if(value.equals("blue")) tokens.add(new Token(Kind.KW_blue,startPos, pos-startPos));
							if(value.equals("green")) tokens.add(new Token(Kind.KW_green,startPos, pos-startPos));
							if(value.equals("alpha")) tokens.add(new Token(Kind.KW_alpha,startPos, pos-startPos));
							if(value.equals("while")) tokens.add(new Token(Kind.KW_while,startPos, pos-startPos));
							if(value.equals("if")) tokens.add(new Token(Kind.KW_if,startPos, pos-startPos));
							if(value.equals("width")) tokens.add(new Token(Kind.KW_width,startPos, pos-startPos));
							if(value.equals("sleep")) tokens.add(new Token(Kind.KW_sleep,startPos, pos-startPos));
						}
						state=State.START;
					}
				}break;
				...
				case HAVE_EQUAL:{
					if(ch=='='){
						...
						throw new LexicalException("wrong input with =",pos);
					}
				}break;
				case HAVE_NOTEQUAL:{
					if(ch=='='){
				...
				case COMMENT:{
					...
					else if(ch=='*' && chars[pos+1]!='/'){
						pos++;
					}
					else if(ch==EOFChar){
						error(pos,line(pos),posInLine(pos),"comment needs to closed");
					}
					else{
						pos++;
					}
